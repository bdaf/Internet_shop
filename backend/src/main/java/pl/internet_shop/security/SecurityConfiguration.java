    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().exceptionHandling()
                ...
                .authorizeRequests((request) -> request.antMatchers( "/api/login").permitAll()
                        ...
                        .antMatchers("/api/products/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/orders").hasAnyAuthority(WORKER,ADMIN,USER)
                        .antMatchers("/api/orders/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/orders/save").hasAuthority(USER)
                        .antMatchers("/api/orders/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/orders/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/products/save/gallery/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/products/{id}/gallery").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/gallery/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/products/{id}/gallery").hasAuthority(ADMIN)
                        .antMatchers("/api/gallery/{id}").hasAuthority(ADMIN)
                        .antMatchers("/api/orders/{id}").hasAuthority(ADMIN)
                        .antMatchers("/api/products/{id}").hasAuthority(ADMIN)
                        .antMatchers("/api/products/{id}").hasAuthority(ADMIN)
                        .antMatchers("/api/producers").permitAll()
                        .antMatchers("/api/producers/save").hasAuthority(ADMIN)
                        .antMatchers("/api/producers/{id}").hasAuthority(ADMIN)
                        .antMatchers("/api/producers/{id}").hasAuthority(ADMIN)
                        ...
                        .antMatchers("/api/categories/{id}").hasAuthority(ADMIN)
                        .antMatchers("/api/categories/{id}").hasAuthority(ADMIN)
