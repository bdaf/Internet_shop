    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().exceptionHandling()
                ...
                .authorizeRequests((request) -> request.antMatchers( "/api/login").permitAll()
                        ...
                        .antMatchers(HttpMethod.DELETE,"/api/gallery/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers(HttpMethod.DELETE,"/api/products/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers(HttpMethod.PUT,"/api/products/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/producers").permitAll()
                        .antMatchers("/api/producers/save").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers(HttpMethod.DELETE,"/api/producers/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers(HttpMethod.PUT,"/api/producers/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/discounts/save/category/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/discounts/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers("/api/galleries").permitAll()
                        .antMatchers("/api/discounts").permitAll()
                        .antMatchers("/api/categories").permitAll()
                        .antMatchers("/api/categories/save").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers(HttpMethod.PUT,"/api/categories/{id}").hasAnyAuthority(WORKER,ADMIN)
                        .antMatchers(HttpMethod.DELETE,"/api/categories/{id}").hasAnyAuthority(WORKER,ADMIN)
